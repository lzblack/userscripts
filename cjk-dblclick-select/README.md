...
恢复 Firefox 双击中文的旧行为。Firefox 116 起切换到 ICU4X 后，双击一个汉字只会选中**单个字**（如双击「今」只选「今」），而不是过去的"选中连续整段汉字到下一个非汉字边界为止"。这个脚本把老行为还回来。
...
1. 检查 `window.getSelection()` 是否含 CJK 汉字。不含 → 不动（保留英文 / 代码的原生行为）
2. 用 `caretPositionFromPoint(x, y)` 定位光标
3. 从光标位置向左、向右扩展，遇到第一个**非 CJK 汉字**字符（U+4E00–9FFF / U+3400–4DBF / U+20000–2A6DF 之外的任何字符）即停止
4. 用扩展后的范围覆盖原选区

"边界"完全由"是否在 CJK 汉字 Unicode 块内"决定，**不维护标点列表**——中英标点、空格、字母数字、Emoji 全部天然作为边界。
